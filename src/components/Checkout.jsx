    ...
    <div>
     ...
     {fill ? (
      ...
      <div style={{ marginTop: "2%" }}>
        ...
        <select id="location" value={locate} onChange={(e) => handleChanges(e)}>
          {details.map((option) => (
      ...
      <div style={{marginTop:"2%"}}>
        {locate!=""?<div style={{border:"1px solid black",backgroundColor:"whitesmoke"}}>
          <h2>Details of location</h2>
          <span><h3>Venue:</h3> {details[locate].name}</span>
          <span><h3>Transport Charge:</h3> {(Number(details[locate].Distance.split('k')[0])*2)*50}</span>
          <span><h3>Distance:</h3> {details[locate].Distance}</span>
        </div>:<></>}
      </div>
